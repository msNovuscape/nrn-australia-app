...
class SendMemberReapplyNotification
{
    /**
    ...
    public function handle($event)
    {
       ...
       if($data['email'] == $event->member->email){
           $data['full_name'] = $name; 
            Mail::send('member_reapply_email', $data, function ($message) use ($member_email,$name) {
                $message->to($member_email, $name)
                ->subject('Request for Reapplication');
            });
        }else
        {
            Mail::send('member_reapply_email', $data, function ($message) use ($user,$member_email,$name) {
                $message->to($member_email,$name);
       ...
       $body = 'After carefully reviewing your application, we insist you to consider submitting another membership application';
